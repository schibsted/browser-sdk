...
import uriBff from './uriBff';
import uri from './uriSpid';
...
export default class SDK {
     ...
     *        The value is TTL in seconds
     * @param {string} [options.env] - possible options [LOCAL, DEV, PRE, PRO, PRO.NO, PRO.COM] where PRO.COM is alias for PRO
     *          by setting this option sdk automatically builds valid uri's to specified environment
     * @param {string} [options.serverUrl] - url to SPiD, by setting this option URL returned from env setting is overridden
     * @param {string} [options.paymentServerUrl] - url to BFF's checkout flow, by setting this option URL returned from env setting is overridden
     */
    constructor(options) {
        // validate options
        ...
        this.uriBuilder = new uri.Builder(uri.normalizeHostname(options.env) || options.serverUrl, {
            client_id: options.client_id,
            redirect_uri: options.redirect_uri
        });
        try {
            this.paymentUriBuilder = new uriBff.Builder(uriBff.normalizeHostname(options.env) || options.paymentServerUrl, {
                client_id: options.client_id,
                redirect_uri: options.redirect_uri
            });
        } catch (e) {
            console.error('Method payment() is not available for provided env.')
        }
    ...
    login(loginType, redirectUri) {
        if (this.options.popup) {
            ...
            const loginPopupRef = openPopup(this.uriBuilder.login(loginType, redirectUri), 'Login');
            if (loginPopupRef) {
        ...
        window.location = this.uriBuilder.login(loginType, redirectUri);
        return window;
    }
    ...
    payment(paylink, redirectUri = null) {
        if (!this.paymentUriBuilder) {
            throw new SDKError('Method is not available');
        }
        if (this.options.popup) {
            if (this._lastPopupRef) {
                this._lastPopupRef.close();
            }
            const popupRef = openPopup(this.paymentUriBuilder.purchasePaylink(paylink, redirectUri), 'Purchase');
            if (popupRef) {
        ...
        window.location = this.paymentUriBuilder.purchasePaylink(paylink, redirectUri);
        return window;
    }
}
