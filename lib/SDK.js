 ...
 *        The value is TTL in seconds
 * @param {string} [options.env] - possible options [LOCAL, DEV, PRE, PRO, PRO.NO]
 *          by setting this option sdk automatically builds valid uri's to specified environment
 * @param {string} [options.serverUrl='LOCAL'] - url to SPiD, by setting this option URL returned from env setting is overridden
 *        Some shorthand strings can be used as well: LOCAL, DEV, PRE, PRO, PRO.NO
 * @param {string} [options.paymentServerUrl='LOCAL'] - url to BFF's checkout flow, by setting this option URL returned from env
...
export default class SDK {
    constructor(options) {
        ...
        if ( typeof options.serverUrl !== 'string' && typeof options.env !== 'string'  ) {
            throw new TypeError('serverUrl or env parameter is required');
        }
        if ( typeof options.paymentServerUrl !== 'string' && typeof options.env !== 'string' ) {
            throw new TypeError('paymentServerUrl or env parameter is required');
        }
    ...
    hasSession() {
        ...
        return this.fetch(this.options.useSessionCluster ? this.uri.sessionCluster(1) : this.uri.session(1))
            .catch(err => {
    ...
    /**
     * Tries to open a popup (if the config asked for it) and if it fails, it uses the redirect flow.
     * If it opens a popup, it tries to keep it on the top by periodically trying to focus it
     * @param {string} title - the popup window title
     * @param {string} popupUrl - a url that will be used for the popup
     * @param {string} [redirectUrl=popupUrl] - a url that will be used for the redirect flow. Defaults to the same url as for the popup
    ...
    /**
     * In order to work correctly this function needs to be called in response to a user
     * event (like click or tap) otherwise the popup will be blocked by the browser's popup blockers
     ...
     * event (like click or tap) otherwise the popup will be blocked by the browser's popup blockers
     * and has to be explicitly authorized to be shown.
     * @param {string} paylink - Identifier of paylink previously created in SPiD
     * @param {string} [redirectUri] - redirection uri where user will be moved after purchase is done
     */
    payment(paylink, redirectUri = null) {
        return this._openPopupOrRedirect('Purchase', this.uri.purchasePaylink(paylink, redirectUri));
    }
